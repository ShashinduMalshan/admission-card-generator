...
The system generates a clean, standardized admission card for every eligible student.

![Admission Card Preview](assets/preview.png)

**Card Layout Explanation:**
Each card includes the following validated sections:

1.  **Header:** Displays the Institute Logo (left) and Document Control details (Doc No, Revision, Issue Date) on the right.
2.  **Title:** "EXAMINATION ADMISSION CARD" in bold, uppercase text.
3.  **Student Details:** A clear table listing the Student Name, ID, Program, Batch, and Module.
4.  **Exam & Eligibility Check:**
      * **Exam Type:** Checkboxes for Written, Practical, and Viva (configured globally).
      * **Validation:** Auto-checked boxes for "Attendance" and "Finance" based on the uploaded data.
5.  **Authorization:** Designated space for the issuer's signature.
